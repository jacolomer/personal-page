...
```{r}
wilcox.test(data$discrimination ~ data$condition, alternative = "two.sided", paired = FALSE)
```
...
#| warning: false
library(infer)
# remotes::install_github("brooke-watson/bplots") # si no está instalado
library(bplots)
library(scales)
```
...
```{r}
cohens_d(data$discrimination, y = data$condition, ci = 0.95, pooled_sd = TRUE)
...
TOSTtwo(
  ...
  n1 = 40, n2 = 40,
  low_eqbound_d = -0.34, high_eqbound_d = 0.34,
...
plot(two.group.meandiff, rawplot.ylabel = "Discrimination scores", rawplot.markersize = 3)
```
...
Primero guardamos el gráfico como un objeto, luego lo exportamos:

```{r}
#| eval: false
library(jpeg)

jpeg(file = "Figure1.jpeg", width = 2000, height = 1400, units = "px", res = 300)
Grafico  # El objeto con el gráfico creado anteriormente
dev.off()
```

También se puede usar `ggsave()` directamente sobre un objeto de `ggplot2`, que es más cómodo:
